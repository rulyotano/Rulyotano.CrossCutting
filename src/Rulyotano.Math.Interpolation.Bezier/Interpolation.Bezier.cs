...
namespace Rulyotano.Math.Interpolation.Bezier
{
    public static class BezierInterpolation
    {
        ...
        /// <returns></returns>
        public static BezierCurve PointsToBezierCurves(IEnumerable<Point> points, bool isClosedCurve, double smoothValue = 0.8)
        {
            var pointList = points.ToList();
            if (pointList.Count < 3)
                return new BezierCurve(new List<BezierCurveSegment>());
            ...
            for (var i = 0; i < pointList.Count - 1; i++)   //iterate for points but the last one
            {
                ...
                if (i == 0) //if is first point
                {
                    if (isClosedCurve)
                    {
                        var previousPoint = pointList[pointList.Count - 2];    //last Point, but one (due inserted the first at the end)
                        x0 = previousPoint.X;
                        y0 = previousPoint.Y;
                    }
                    else    //Get some previous point
                    {
                        var previousPoint = pointList[i];  //if is the first point the previous one will be it self
                        x0 = previousPoint.X;
                        y0 = previousPoint.Y;
                    }
                }
                ...
                {
                    x0 = pointList[i - 1].X;   //Previous Point
                    y0 = pointList[i - 1].Y;
                }

                double x3, y3;

                if (i == pointList.Count - 2)    //if is the last point
                {
                    if (isClosedCurve)
                    {
                        var nextPoint = pointList[1];  //second Point(due inserted the first at the end)
                        x3 = nextPoint.X;
                        y3 = nextPoint.Y;
                    }
                    else    //Get some next point
                    {
                        var nextPoint = pointList[i + 1];  //if is the last point the next point will be the last one
                        x3 = nextPoint.X;
                        y3 = nextPoint.Y;
                    }
                }
                ...
                (
                    ...
                    secondControlPoint: i == pointList.Count - 2 && !isClosedCurve ? new Point(x2, y2) : new Point(ctrl2X, ctrl2Y),
                    endPoint: new Point(x2, y2)
                ));
            }

            return new BezierCurve(toRet);
        }
    }
}
